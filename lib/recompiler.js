...
var processModule = function(module) {
	...
	if (fs.existsSync(fn) && module.meta && module.meta.repository && module.meta.repository.url){
		...
		if (fs.existsSync(path.join(tmp, "binding.gyp"))){
			...
			batch.script.push({cmd:cmd});*/
			/*
			cmd = "cd " + tmp + " && node-pre-gyp rebuild";
			batch.script.push({cmd:cmd});*/

			cmd = 'cd ' + tmp + ' && set module_path=./out/sadasd && set module_name=tadaaa && set HOME=' + electronGypHome + ' && node-gyp rebuild --target=' + module.config.electronVersion + ' --arch=' + module.config.arch + ' --dist-url=https://atom.io/download/atom-shell';
			if (module.meta.name === "sqlite3"){
				// FIXME : how to determine module name ? 
				cmd += ' --module_name=node_sqlite3 --module_path=build/release';
			}
			batch.script.push({cmd:cmd});
